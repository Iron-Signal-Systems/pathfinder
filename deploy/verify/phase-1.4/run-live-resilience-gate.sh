...
BUILD_ROOT="/root/pathfinder-build/phase-1.4-resilience"
...
TEST_BINARY="${BUILD_ROOT}/pathfinder-phase14-resilience.test"
ARTIFACTD_BINARY="${BUILD_ROOT}/pathfinder-artifactd-phase14-resilience"
JAIL_TEST_BINARY="${JAIL_ROOT}${TEST_ROOT}/pathfinder-phase14-resilience.test"
JAIL_ARTIFACTD_BINARY="${JAIL_ROOT}${TEST_ROOT}/pathfinder-artifactd-phase14-resilience"
...
cleanup()
{
    ...
    if [ "${status}" -ne 0 ]; then
        echo
        echo "===== PHASE 1.4 LIVE RESILIENCE GATE FAILED ====="
    fi
...
echo "===== PHASE 1.4 LIVE RESILIENCE GATE ====="
...
echo
echo "===== PROVE ACCEPTED LIFECYCLE HELPER ====="

LIFECYCLE_HELPER_SHA="$(
    sha256 -q "${GO_ROOT}/cmd/pathfinder/phase14_lifecycle_integration_test.go"
)"
echo "${LIFECYCLE_HELPER_SHA}"

if [ "${LIFECYCLE_HELPER_SHA}" != "${EXPECTED_LIFECYCLE_HELPER}" ]; then
    echo "FAIL: lifecycle helper differs from accepted gate."
    exit 1
fi
...
HBA_BACKUP="${HBA_FILE}.phase14-resilience.$$"
...
HBA_TMP="${HBA_FILE}.phase14-resilience.$$.tmp"

{
    echo "# Pathfinder Phase 1.4 disposable resilience gate - temporary"
...
if [ "${HBA_MATCHES}" != "1" ]; then
    echo "FAIL: temporary resilience pg_hba rule was not proven."
    exit 1
fi
...
$1 == "readiness_listen" {
    print "readiness_listen", "127.0.0.1:18082"
    next
}
$1 == "state_dir" {
...
jexec pfapp /usr/sbin/daemon \
    ...
    -u pfartifact \
    "${TEST_ROOT}/pathfinder-artifactd-phase14-resilience" \
...
echo
echo "===== RUN REAL RESILIENCE TEST ====="

if ! jexec -U pathfinder pfapp env \
    PATHFINDER_PHASE14_RESILIENCE=YES \
    PATHFINDER_PHASE14_RESILIENCE_CONFIG="${TEST_ROOT}/pathfinder.conf" \
    "${TEST_ROOT}/pathfinder-phase14-resilience.test" \
    -test.run '^TestPhase14LiveResilience$' \
    -test.v; then
...
echo
echo "===== DISPOSABLE DATABASE FINAL PROOF ====="
...
echo "${DISPOSABLE_COUNTS}"

if [ "${DISPOSABLE_COUNTS}" != "5|5|4|2|1|2|2" ]; then
    echo "FAIL: unexpected disposable resilience database counts."
    exit 1
fi
...
echo
echo "PASS: completed retry created no duplicate run."
echo "PASS: malformed artifact produced FAILED with zero semantics/checkpoint."
echo "PASS: stale unresolved run became INTERRUPTED and retry used a new UUIDv7."
echo "PASS: cross-artifact SourceRecordProcessing linkage was rejected."
...
echo
echo "===== PHASE 1.4 LIVE RESILIENCE GATE: PASS ====="
